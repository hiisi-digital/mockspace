...
#[test]
it_runs_the_ignored_generation_idempotence_suite() {
    assert_ok cargo test -p mockspace --test generation_idempotent -- --ignored
}

# Added the same day the suite below was written and this line was not, which
# is the identical mistake this file's own header describes. A discriminating
# arm nothing runs is a claim, not a check.
#[test]
