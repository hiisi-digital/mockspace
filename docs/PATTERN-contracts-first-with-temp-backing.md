...
**Bench harness.** Variant cdylibs export four named symbols (`bench_abi_hash`, `bench_name`, `bench_entry`, `bench_output_size`) per `bench-core::{AbiHashFn, BenchNameFn, BenchEntryFn, OutputSizeFn}`. The host dlsyms each via `libloading`, checks the FNV-1a ABI hash, checks the variant's output size against the routine's output buffer, dispatches the entry function. Wire shape is `#[repr(C)] FfiBenchCall`. This is the workspace's canonical cdylib-loading pattern. See `bench-core/src/lib.rs:320-358` and `bench-harness/src/harness.rs:90-120`.
