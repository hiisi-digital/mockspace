...
pub fn plan(cfg: &Config, crates: &crate::model::CrateMap) -> Vec<Planned> {
    ...
    for tmpl in design_templates(cfg) {
        let id = match tmpl.owner {
            // Root templates. `DESIGN.md.tmpl` is one of these rather than a
            // special case: it differs only in the placeholder it expands, and
            // a placeholder is not a reason for a second code path.
            None => {
                let name = tmpl
                    .path
                    .file_name()
                    ...
                    .unwrap_or_default();
                DocId::root(name.trim_end_matches(".tmpl"), cfg)
            },
            Some(ref crate_name) => {
                ...
                DocId::Crate {
                    upper,
                    subject: tmpl.subject.clone(),
                    depth,
                }
            },
        };
        out.push(Planned::template(id, tmpl.path));
    }

    out
}

/// One design document's template, and which crate it belongs to.
///
/// `owner` is the crate directory's name, or `None` for a template sitting at
/// the mock root. `subject` is `OVERVIEW` for a crate's `DESIGN.md.tmpl` and the
/// upper-cased stem for a deep dive; it says nothing about a root template.
...
/// Every template the design tier is written in, in render order.
///
/// The one enumeration of what a design document is. [`plan`] turns each into a
/// document to render and the nuke turns each into a file to wipe, so a
/// template neither of them can see is a template neither renders nor clears.
/// Two walks would drift, and the direction they would drift in is a nuke that
/// leaves part of the tier standing while reporting that it went.
///
/// `mock/agent/` is deliberately not here. Those templates generate the
/// assistant instructions, which are not the design of anything and are
/// regenerated from the tool rather than rewritten from a canon.
pub fn design_templates(cfg: &Config) -> Vec<DesignTemplate> {
    let mut out = Vec::new();
    ...
    // Per-crate overviews and deep dives, across every source directory.
    // Walking one group would leave every crate in the others with no generated
    // document at all, and the run would report success.
    for dir in crate::parse::package_dirs_in(&cfg.src_dirs) {
        let crate_name = dir
            .file_name()
            .map(|n| n.to_string_lossy().to_string())
            .unwrap_or_default();
