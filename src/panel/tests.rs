    ...
    fn seat_99_may_mint_and_seat_100_may_not() {
        ...
        assert_eq!(next_seat_number(&inv), Some(99), "seat 99 must still be mintable");
        inv.seats.push(seat(99));
        assert_eq!(next_seat_number(&inv), None, "seat 100 must never be offered");
    }
    ...
    fn minting_is_refused_once_the_cadence_is_met_and_recovers_after_consolidating() {
        ...
        assert_eq!(inv.seats.len(), 3, "the refused seat must not have been appended");
