...
fn collects_nested_bench_and_sketch_targets_only() {
    ...
    assert!(
        rels.contains(&"mock/benches/variants/foo/target".to_string()),
        "got: {rels:?}"
    );
    assert!(
        rels.contains(&"mock/benches/engine_vs_std/target".to_string()),
        "got: {rels:?}"
    );
    assert!(
        rels.contains(&"mock/research/sketches/202601010000_x/target".to_string()),
        "got: {rels:?}"
    );
    assert!(
        rels.contains(&"crates/bar/tests/fixture_crate/target".to_string()),
        "got: {rels:?}"
    );
    assert!(
        !rels.contains(&"target".to_string()),
        "must not collect repo-root target: {rels:?}"
    );
    assert!(
        !rels.contains(&"mock/target".to_string()),
        "must not collect mockspace install: {rels:?}"
    );
...
fn spares_research_targets_outside_sketches() {
    ...
    assert_eq!(
        rels,
        vec!["mock/research/sketches/202601010000_x/target".to_string()],
        "got: {rels:?}"
    );
}
...
fn does_not_descend_into_target_dirs() {
    ...
    assert_eq!(
        found.len(),
        1,
        "should stop at first target dir, got: {found:?}"
    );
