...
/// The line every generation header opens with, whatever comment syntax
/// carries it. One spelling, so the header and the check that recognises it
/// cannot drift apart.
pub const GENERATED_MARKER: &str = "AUTO-GENERATED: DO NOT EDIT DIRECTLY";

/// How far into a file the marker is looked for.
///
/// Two, because that is what the header forms need: the DOT one opens with it
/// and the markdown and SVG ones open a comment first. Anything looser matches
/// a document that quotes the marker in prose a line or two in, and this
/// crate's own documentation does exactly that.
///
/// `every_header_puts_the_marker_inside_the_window` holds the three builders
/// to it, so widening a header without widening this fails there.
const MARKER_WITHIN_LINES: usize = 2;

/// Whether this content was written by a generation path.
///
/// The question the docs sweep asks before removing anything: a file carrying
/// the marker is one of ours, and everything else belongs to whoever put it
/// there. Reading the file is what makes that safe without a record of a
/// previous run, which can be deleted, can be stale, and is one more thing to
/// keep honest.
pub fn is_generated(content: &str) -> bool {
    content
        .lines()
        .take(MARKER_WITHIN_LINES)
        .any(|l| l.contains(GENERATED_MARKER))
}
...
pub fn generation_header_md(cfg: &Config) -> String {
    ...
    writeln!(hdr, "<!--").unwrap();
    writeln!(hdr, "  {GENERATED_MARKER}").unwrap();
    writeln!(hdr, "").unwrap();
...
mod tests {
    ...
    #[test]
    fn every_header_puts_the_marker_inside_the_window() {
        // What makes the docs sweep able to tell its own output from a
        // hand-written file. Each builder is asked directly rather than the
        // property being asserted about one of them and assumed for the rest:
        // the SVG and DOT forms differ from the markdown one in exactly the
        // way that would move the marker out of the window.
        let cfg = Config::from_dir(Path::new("/nonexistent"));
        for (what, header) in [
            ("markdown", generation_header_md(&cfg)),
            ("dot", generation_header_dot(&cfg)),
            ("svg", generation_header_svg(&cfg)),
        ] {
            assert!(
                is_generated(&header),
                "the {what} header does not read as generated:\n{header}"
            );
            // and it is the marker doing it, at a line the window covers,
            // rather than something else in the header happening to match.
            let at = header
                .lines()
                .position(|l| l.contains(GENERATED_MARKER))
                .unwrap_or_else(|| panic!("{what} header has no marker:\n{header}"));
            assert!(at < MARKER_WITHIN_LINES, "{what} marker at line {at}");
        }
    }

    #[test]
    fn a_file_that_only_talks_about_the_marker_is_not_generated() {
        // The control. Without it the check above passes for a predicate that
        // returns true for everything, and a documentation page explaining the
        // header is the file this crate is most likely to be pointed at.
        let prose = format!(
            "# Usage\n\nEvery generated file opens with `{GENERATED_MARKER}`, so you can tell.\n"
        );
        assert!(!is_generated(&prose));
        assert!(!is_generated(""));
        assert!(!is_generated("# A document\n\nwith no header at all.\n"));

        // and a header two lines further down is still not one: the window is
        // the whole mechanism, so a version that scanned the file would pass
