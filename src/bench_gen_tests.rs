...
// --- the branch the generated crates build against ------------------------------

const TIP: &str = "0123456789abcdef0123456789abcdef01234567";
const LATER: &str = "fedcba9876543210fedcba9876543210fedcba98";

#[test]
fn the_default_branch_pin_reaches_every_generated_manifest_as_the_tip() {
    // The defect: the driver's lockfile held the tip of its first build
    // while the support crates moved on, and the run died on a manifest
    // key the old harness had never heard of.
    let mock = tempfile::tempdir().unwrap();
    let asked = std::cell::Cell::new(0);
    let resolve = |url: &str, branch: &str| {
        asked.set(asked.get() + 1);
        assert_eq!(url, "https://github.com/hiisi-digital/mockspace");
        assert_eq!(branch, "dev");
        Ok(TIP.to_string())
    };
    let dep = pinned_dep(DEFAULT_MOCKSPACE_DEP, mock.path(), &resolve);
    assert_eq!(asked.get(), 1);
    assert!(dep.contains(&format!("rev = \"{TIP}\"")), "{dep}");
    assert!(!dep.contains("branch"), "{dep}");

    let driver = driver_cargo_toml(&dep, &[]).unwrap();
    let arm = ArmSource {
        bench:        "hash".into(),
        arm:          "fnv".into(),
        dir:          PathBuf::from("/tree/hash/arms/fnv"),
        has_manifest: false,
    };
    let arm = arm_cargo_toml(&arm, &dep, &[]).unwrap();
    for (what, toml) in [("driver", &driver), ("arm", &arm)] {
        assert!(toml.contains(TIP), "the {what} is not at the tip: {toml}");
        assert!(!toml.contains("branch = \"dev\""), "the {what} still floats: {toml}");
    }
}

#[test]
fn the_resolution_is_kept_under_the_driver_directory() {
    let mock = tempfile::tempdir().unwrap();
    pinned_dep(DEFAULT_MOCKSPACE_DEP, mock.path(), &|_: &str, _: &str| Ok(TIP.into()));
    let pins = driver_gen_dir(mock.path()).join(".pack-pins");
    assert!(pins.is_dir(), "no cache at {}", pins.display());
    // A second run within the hour takes the kept tip rather than
    // asking again, which is what keeps a bench run off the network.
    let again = pinned_dep(DEFAULT_MOCKSPACE_DEP, mock.path(), &|_: &str, _: &str| {
        Ok(LATER.into())
    });
    assert!(again.contains(TIP), "{again}");
}

#[test]
fn a_rev_or_a_path_is_handed_through_untouched_and_nothing_is_asked() {
    let mock = tempfile::tempdir().unwrap();
    let never = |_: &str, _: &str| -> Result<String, String> {
        panic!("a spec with no branch was resolved")
    };
    for spec in [
        "{ git = \"https://github.com/hiisi-digital/mockspace\", rev = \"abc\" }",
        "{ path = \"/repo/mockspace\" }",
    ] {
        assert_eq!(pinned_dep(spec, mock.path(), &never), spec);
    }
}

#[test]
fn an_unreachable_remote_with_no_kept_tip_leaves_the_branch_to_cargo() {
    let mock = tempfile::tempdir().unwrap();
    let dep = pinned_dep(DEFAULT_MOCKSPACE_DEP, mock.path(), &|_: &str, _: &str| {
        Err("offline".into())
    });
    assert_eq!(dep, DEFAULT_MOCKSPACE_DEP);
}
