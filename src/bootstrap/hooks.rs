...
/// hook leaves open. The check is pure bash with no `cargo mock` dependency, so
/// it holds even when the mockspace proxy is absent or broken. Byline
/// attribution is reserved for genuinely headless autonomous work; a commit
/// made under human direction carries none. Human co-authors are kept.
...
# Byline safety net: scan every pushed commit message for an agent authorship
# byline or a tool-advertising trailer. The commit-msg hook catches these at
# commit time; this catches anything that reached the branch another way (a
# different clone, an amend that skipped the hook). Human co-authors are kept.
PUSH_MSGS=""
...
mod byline_hook_tests {
    ...
    /// Generate the commit-msg hook, run it against `msg`, return the exit code.
    fn run_commit_msg(msg: &str) -> i32 {
        let n = SEQ.fetch_add(1, Ordering::Relaxed);
        let dir = std::env::temp_dir().join(format!("mock_byline_{}_{n}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let hook = dir.join("commit-msg");
        // A user-hook path that does not exist so the sourced preamble no-ops.
        let content = gen_commit_msg(&dir.join("no-user-hook"));
        ...
        let code = Command::new(&hook).arg(&msgfile).status().unwrap().code().unwrap_or(-1);
