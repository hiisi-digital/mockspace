...
pub(crate) fn ensure_generated_hooks(repo_root: &Path, mock_dir: &Path, actions: &mut Vec<String>) {
    ...
    // The repository's own hooks under .git/hooks/, which the generated hooks
    // run at their tail so whatever another tool installed there still fires.
...
pub(crate) fn gen_commit_msg(user_hook: &Path) -> String {
    let chain = chain_user_hook(user_hook, false);
    let byline = message_commit_msg_body();

    format!(
        r##"#!/usr/bin/env bash
{MANAGED_MARKER}
# Generated by mockspace. The repository's own .git/hooks/commit-msg runs after it.
...
/// The shell function every pass through a generated hook ends in: the
/// repository's own `.git/hooks/<name>` when it exists and is executable, with
/// the same arguments and, where `replay_stdin`, the refs git handed the hook
/// piped back in, and its exit status as the hook's.
///
/// It ran first, once. That put a hook that reads stdin, which a pre-push gate
/// does, ahead of this hook's own capture of it, so the refs went to whichever
/// of the two read them first and the other scanned nothing. Running after
/// the gate, on the captured refs, is what mockspace ruled for its durable gate
/// too: its own checks, then whatever the repository keeps there, and nothing
/// past a refusal.
pub(crate) fn chain_user_hook(user_hook: &Path, replay_stdin: bool) -> String {
    let path = user_hook.display();
    let replay = if replay_stdin { "printf '%s\\n' \"$PREPUSH_STDIN\" | " } else { "" };
    format!(
        r#"# The repository's own hook, once validation has passed: same arguments,
# same stdin, its exit status. Absent or not executable, the pass stands.
USER_HOOK="{path}"
ms_chain() {{
    [ -x "$USER_HOOK" ] || exit 0
    {replay}"$USER_HOOK" "$@"
...
pub(crate) fn gen_pre_commit(mock_rel: &str, user_hook: &Path) -> String {
    ...
    format!(
        r##"#!/usr/bin/env bash
{MANAGED_MARKER}
# Generated by mockspace. The repository's own .git/hooks/pre-commit runs after it.
...
pub(crate) fn gen_pre_push(mock_rel: &str, user_hook: &Path) -> String {
    let chain = chain_user_hook(user_hook, true);
    let byline = message_prepush_scan_body();

    format!(
        r##"#!/usr/bin/env bash
{MANAGED_MARKER}
# Generated by mockspace. The repository's own .git/hooks/pre-push runs after it.
...
mod byline_hook_tests {
    ...
    /// The repository's own hook, under `.git/hooks/<name>`: records its
    /// arguments and its stdin to `own.log` at the repo root, then exits `code`.
    fn plant_own_hook(repo: &Path, name: &str, code: i32) -> PathBuf {
        ...
        std::fs::write(
            &path,
            format!(
                "#!/usr/bin/env bash\nlog=\"$(git rev-parse --show-toplevel)/own.log\"\n\
                 printf 'args:%s\\n' \"$*\" >> \"$log\"\ncat >> \"$log\"\nexit {code}\n"
            ),
        )
        .unwrap();
    ...
    fn own_log(repo: &Path) -> Option<String> {
        std::fs::read_to_string(repo.join("own.log")).ok()
    }

    fn refs_line(sha: &str) -> String {
        format!("refs/heads/main {sha} refs/heads/main {}\n", "0".repeat(40))
    }

    /// The durable pre-push over a repo carrying an own hook that exits 42,
    /// with `config` as its `mockspace.toml` where given and `generated` as an
    /// executable generated hook where given. Returns the exit code and the
    /// own hook's log, if it ran.
    fn durable_with_own_hook(
        config: Option<&str>,
        generated: Option<&str>,
    ) -> (i32, Option<String>) {
        let (repo, sha) = repo_with_commit("fix: whatever");
        if let Some(c) = config {
            std::fs::write(repo.join("mockspace.toml"), c).unwrap();
        }
        ...
        plant_own_hook(&repo, "pre-push", 42);
        let code = run_script(
            &mockspace_manifest::gate::durable_hook("pre-push", HOOK_VERSION),
            &[],
            &refs_line(&sha),
        ...
        let log = own_log(&repo);
        let _ = std::fs::remove_dir_all(&repo);
        (code, log)
    }

    #[test]
    fn durable_chains_to_the_repositorys_own_hook_where_it_governs_nothing() {
        // No mockspace.toml, so the gate has nothing to say; the hook under
        // .git/hooks, which git never reads while core.hooksPath points at the
        // gate, still runs, on the same refs, and its exit is the push's.
        let (code, log) = durable_with_own_hook(None, None);
        assert_eq!(code, 42, "the own hook's exit status is the hook's");
        let log = log.expect("the own hook ran");
        assert!(
            log.contains("refs/heads/main"),
            "the refs reached it: {log}"
        );
    }

    #[test]
    fn durable_chains_after_passing_an_uninitialised_project_outside_the_surface() {
        let (code, log) = durable_with_own_hook(Some("mock_dir = \"mock\"\n"), None);
        assert_eq!(code, 42);
        assert!(log.is_some_and(|l| l.contains("refs/heads/main")));
    }

    #[test]
    fn durable_never_chains_past_a_block() {
        let (code, log) = durable_with_own_hook(
            Some("mock_dir = \"mock\"\nuninitialised_blocks = \"all\"\n"),
            None,
        );
    ...
    #[test]
    fn durable_leaves_the_chain_to_the_generated_hook_when_initialised() {
        // The generated hook chains at its own tail; a stub that does not is
        // how the durable gate is shown not to chain a second time.
        let (code, log) = durable_with_own_hook(
            Some("mock_dir = \"mock\"\n"),
            Some("#!/usr/bin/env bash\nexit 0\n"),
        );
        assert_eq!(code, 0);
        assert!(log.is_none(), "the delegate owns the chain");
    }

    #[test]
    fn the_generated_pre_commit_chains_on_the_pass_with_nothing_staged() {
        // The early pass, which used to be a bare exit, is where a commit with
        // nothing on the design surface leaves; the own hook runs there too.
        let (repo, _) = repo_with_commit("fix: whatever");
        let own = plant_own_hook(&repo, "pre-commit", 42);
        let hook = gen_pre_commit("mock", &own);
        let code = run_script(&hook, &[], "", Some(&repo), Some(&launcher_free_path()));
        assert_eq!(code, 42, "the own hook's exit is the commit's");
        assert!(
            own_log(&repo).is_some_and(|l| l.starts_with("args:\n")),
            "no arguments, none forwarded"
        );
        // and an own hook that passes leaves the commit passing
        plant_own_hook(&repo, "pre-commit", 0);
        let _ = std::fs::remove_file(repo.join("own.log"));
        assert_eq!(
            run_script(&hook, &[], "", Some(&repo), Some(&launcher_free_path())),
            0
        );
        assert!(own_log(&repo).is_some());
        // and none at all is the pass as it was
    ...
    /// A PATH whose `mock` and `cargo` are stubs: the launcher swallows the
    /// batch it is handed, and `cargo mock` appends `gate` to the own log so the
    /// order of the gate and the own hook can be read off one file.
    fn stubbed_path(repo: &Path) -> String {
        ...
        for (name, body) in [
            ...
            (
                "cargo",
                "#!/usr/bin/env bash\necho gate >> \"$(git rev-parse --show-toplevel)/own.log\"\nexit 0\n",
            ),
        ] {
            let p = bin.join(name);
        ...
        format!("{}:{}", bin.display(), launcher_free_path())
    }

    #[test]
    fn the_generated_pre_push_hands_the_captured_refs_to_the_own_hook_after_the_gate() {
        // The own hook ran first once, ahead of this hook's capture of stdin,
        // so a hook that reads the refs, which a release gate does, took them
        // from the scan. Now the scan captures, the gate runs, and the own hook
        // gets the same refs replayed.
        let (repo, sha) = repo_with_commit("fix: whatever");
        std::fs::write(repo.join("mockspace.toml"), "mock_dir = \"mock\"\n").unwrap();
        let own = plant_own_hook(&repo, "pre-push", 0);
        let hook = gen_pre_push("mock", &own);
        let path = stubbed_path(&repo);
        let code = run_script(&hook, &[], &refs_line(&sha), Some(&repo), Some(&path));
        let log = own_log(&repo).unwrap_or_default();
        assert_eq!(code, 0, "{log}");
        let gate = log.find("gate\n").expect("the gate ran");
        let own_at = log.find("args:").expect("the own hook ran");
        assert!(gate < own_at, "the gate runs before the own hook: {log}");
        assert!(
            log[own_at ..].contains(&format!("refs/heads/main {sha}")),
            "the refs were replayed: {log}"
        );
        // and the own hook's refusal is the push's
        plant_own_hook(&repo, "pre-push", 42);
        assert_eq!(
            run_script(&hook, &[], &refs_line(&sha), Some(&repo), Some(&path)),
            42
        );
        let _ = std::fs::remove_dir_all(&repo);
    }
    ...
    #[test]
    fn the_generated_commit_msg_hands_the_message_file_to_the_own_hook_after_the_lints() {
        let (repo, _) = repo_with_commit("fix: whatever");
        let msgfile = repo.join("COMMIT_EDITMSG");
        std::fs::write(&msgfile, "fix: a message\n").unwrap();
        let own = plant_own_hook(&repo, "commit-msg", 42);
        let hook = gen_commit_msg(&own);
        let path = stubbed_path(&repo);
        let code = run_script(&hook, &[&msgfile], "", Some(&repo), Some(&path));
        assert_eq!(code, 42);
        let log = own_log(&repo).unwrap_or_default();
        assert!(
            log.contains(&format!("args:{}", msgfile.display())),
            "{log}"
        );
        let _ = std::fs::remove_dir_all(&repo);
    }
