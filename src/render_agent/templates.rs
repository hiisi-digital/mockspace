    ...
    if cfg.agent.panel_discipline {
        rules.push(BuiltinRule {
            ...
            body: format!(
                ...
                "Every seat is minted, never counted or guessed. `mock panel seat <slug> <persona> <topic...>` allocates the next seat number from the panel's own inventory file, one past whatever is already recorded there. Two dispatches minting at once cannot collide, because the number always comes from the record rather than from a caller's own tally. Ninety-nine is the last seat a panel may ever mint; past it, the panel is over, not paused, and the next step is closing it or opening a new one for whatever remains.",
