        ...
        BuiltinRule {
            ...
            body: substitute_builtin_vars(
                concat!(
                    ...
                    "each round left it is not canon; it is drift with a confident name.\n",
                    "\n",
                    ...
                    "way a design round closes: archived under `{mock_dir}/canon/` with a timestamp, so the ",
                    "superseded canon is dated and kept, not overwritten in place.\n",
                    "\n",
                    ...
                    "meant to.\n",
                    "\n",
                    "### The directory is reserved; the guard is not built yet\n",
                    "\n",
                    "`{mock_dir}/canon/` is reserved so the mutation order above has an address to enforce ",
                    "itself against, not so it is enforced today. The eventual guard refuses an edit to ",
                    "`{mock_dir}/canon/` while any design document remains under `{mock_dir}/crates/`, which is ",
                    "the mechanical form of \"change the canon only after nuking every design.\" **That guard ",
                    "does not exist yet.** No lint, phase gate, or hook currently reads or restricts this ",
                    "directory; nothing stops an edit to it today. Treat the directory as a reservation, not as ",
                    "a working gate, until a later rule says otherwise.\n",
                ),
                &mock_rel, project_name, crate_prefix,
            ),
        },
