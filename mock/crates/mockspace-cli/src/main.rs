...
use mockspace_rs::{
    ...
    CloseMetadata, DefaultBranchName, DesignRound, Finding, FixOpts, FlockTransitionLock, Gate,
    LintCfgStore, LintEngine, LockError, Namespace, ObjectId, Phase, RegenerateError,
    RegenerateReport, RepoError, RepoHandle, ReplanMode, RoundState, RunSurface, Severity,
    DefaultSlug, DefaultTaskId, TaskMeta, TaskResolution, WriteState,
};
...
enum TaskVerb {
    ...
    Close {
        ...
        /// the [`DefaultBranchName`] validator (subset of
        /// `git-check-ref-format`).
        #[arg(long, value_parser = parse_branch_name)]
        branch: Option<DefaultBranchName>,
...
/// Parse a git branch name into [`DefaultBranchName`] with a
/// CLI-friendly error message. Used as a clap `value_parser` for
/// flags that name a git branch (e.g. `mock task close --branch=<name>`).
fn parse_branch_name(raw: &str) -> Result<DefaultBranchName, String> {
    DefaultBranchName::new(raw).map_err(|e| format!("invalid branch name `{raw}`: {e}"))
}
...
fn run_task_close(
    ...
    // CloseMetadata.closed_branch is now Option<DefaultBranchName>
    // per #601. The remaining string fields (closing_phase,
    // closing_round_slug) collapse to String here at the CLI/IO
    // boundary until #595's IO-carrier retyping lands. The typed
    // values survive up to this point; downstream IO writes them
    // into TaskClosure's still-String wire format.
